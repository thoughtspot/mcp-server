...
export class StorageServiceClient {
	...
	/**
	 * Persist the full state of a Spotter model-creation session, keyed by its model_session_id.
	 * Overwrites any existing state for that session.
	 */
	...
	/**
	 * Retrieve the state of a model-creation session, or null if it does not exist / has expired.
	 */
	...
	/**
	 * Append streamed model updates for a session. `resetDone` clears the done flag for a new turn;
	 * `isDone` marks the current turn complete. Safe to call repeatedly as the stream is consumed.
	 */
	...
	/**
	 * Retrieve all model updates appended since the last call (advances a per-session read
	 * bookmark), plus whether the current turn is done. Poll this after send_model_message.
	 */
