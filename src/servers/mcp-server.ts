...
const CreateLiveboardSchema = z.object({
    ...
    noteTile: z.string().describe(`
        ...
        The output format is an html note tile which can have custom styles and formatting. Follow these STRICT SPACING RULES:
        ...
        <p class=\"theme-module__editor-paragraph\" dir=\"ltr\">
            <span style=\"white-space: pre-wrap;\">
                summary of analysis done and answers to queries asked by user. Use colors and styles in the html to make it more readable and visually appealing.
                Also add the date and time of the analysis to the note tile in the footer of the note tile. The format should be like this: Generated on <date> <time>
            </span>
        </p>
        <div class=\"pinboard-note-tile-module__noteTileBg editor-module__bgNode\"></div>

        MANDATORY RULES:
        1. Never use padding > 8px anywhere
        2. Never use margin > 5px anywhere  
        3. Always specify margin: 0 for headings and paragraphs, then add only minimal bottom margin if needed
        5. Use line-height: 1.2 maximum
        6. Remove default spacing with explicit margin: 0 declarations
        ` ),
