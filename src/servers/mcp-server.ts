...
const CreateLiveboardSchema = z.object({
    ...
    summary: z.string().describe("A comprehensive, detailed and elaborate summary of the answers to the queries asked by the user along with all the analysis done on the data. This should be within 200 words. Do not add any other text to the summary."),
});
...
export class MCPServer extends Server {
    ...
    async callCreateLiveboard(request: z.infer<typeof CallToolRequestSchema>) {
        const { name, answers, summary } = CreateLiveboardSchema.parse(request.params.arguments);
        const liveboard = await this.getThoughtSpotService().fetchTMLAndCreateLiveboard(name, answers, summary);
        const span = getActiveSpan();
