...
export const FetchDataInputSchema = z.object({
	...
	visualization_ids: z
		...
		.optional()
		.describe(
			"When `object_id` is a Liveboard, restrict the fetch to these visualization GUIDs (use to pull a single visualization/answer pinned on a Liveboard). Omit to fetch every visualization on the Liveboard. Ignored for standalone Answers.",
		),
	max_rows: z
		.number()
		.int()
		...
		.optional()
		.describe(
			"Maximum number of rows to return per visualization. Defaults to 1000. Raise this only when you need the full result set and the object is known to be large.",
		),
});
