...
class ApiHandler {
	...
	private getThoughtSpotService(props: Props): ThoughtSpotService {
		...
		return new ThoughtSpotService(
			getThoughtSpotClient(props.instanceUrl, props.accessToken),
		);
	}
	...
	async getRelevantQuestions(
		...
		return await service.getRelevantQuestions(
			query,
			datasourceIds,
			additionalContext || "",
		);
	...
	async createLiveboard(
		...
		const service = this.getThoughtSpotService(props);
		const result = await service.fetchTMLAndCreateLiveboard(
			name,
			answers,
			noteTileParsedHtml,
		);
...
apiServer.post(
	...
	async (c) => {
		...
		const questions = await handler.getRelevantQuestions(
			props,
			query,
			datasourceIds,
			additionalContext,
...
apiServer.post(
	...
	async (c) => {
		...
		const liveboardUrl = await handler.createLiveboard(
			props,
			name,
			answers,
			noteTile,
