...
// Spotter model-creation session state is stored as a single blob under this key. It lives in a
// separate DO instance (routed by model_session_id) from analytical conversations, so there is no
// collision with the message/bookmark keys above.
const MODEL_SESSION_KEY = "model-session";

// Streamed model updates use their own write/read bookmarks and per-index keys (independent of the
// analytical-message ones above) so the background stream consumer can append while
// get_model_updates reads — each op is atomic within the DO, so no read-modify-write clobbering.
...
export class ConversationStorageServerSQLite {
	...
	/*
	 * Store the full model-session state as a single blob and restart the TTL. Overwrites any
	 * previous state for this session (the caller owns read/write ordering).
	 */
	...
	/*
	 * Retrieve the model-session state, or null if this session was never created / has expired.
	 */
	...
	/*
	 * Append streamed model updates starting at the current MODEL_WRITE_BOOKMARK, advancing it.
	 * `resetDone` clears the done flag at the start of a new turn (done in the same batch as the
	 * first appended updates, so a racing reader never sees the previous turn's done). `isDone`
	 * marks the turn complete and is always written in the same (last) batch as its updates, so a
	 * reader never observes done before the final updates are visible.
	 */
	...
	/*
	 * Return all model updates appended since the last call, advancing MODEL_READ_BOOKMARK. Mirrors
	 * getNewMessagesAndUpdateBookmark: the read is atomic within the DO, so it is safe to poll this
	 * while the background stream consumer is still appending.
	 */
	private async getNewModelUpdatesAndAdvanceBookmark(): Promise<ModelUpdatesState> {
		...
		if (writeBookmark > readBookmark) {
			...
			for (let i = readBookmark; i < writeBookmark; i++) {
				const update = updatesMap.get(MODEL_UPDATE_KEY_PREFIX + i);
				if (update) updates.push(update);
			}
			// Only advance the read bookmark when it actually moved — an idle poll (no new
			// updates) skips the write, so it costs a single read and no DO write contention.
