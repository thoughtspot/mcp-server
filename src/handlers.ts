...
app.post("/authorize", async (c) => {
    return withSpan('authorize', async (span) => {
        try {
            ...
            span.setStatus({ code: SpanStatusCode.ERROR, message: 'Error in POST /authorize' });
...
app.get("/callback", async (c) => {
    return withSpan('callback', async (span) => {
        ...
        try {
            ...
            span.setStatus({ code: SpanStatusCode.ERROR, message: 'Error decoding OAuth request info' });
...
app.post("/store-token", async (c) => {
    return withSpan('store-token', async (span) => {
        let token: string;
        ...
        try {
            ...
            span.setStatus({ code: SpanStatusCode.ERROR, message: 'Error parsing JSON in store-token' });
            console.error('Error parsing JSON in store-token:', error);
            return c.text('Invalid JSON format', 400);
        }
        
        if (!token || !oauthReqInfo || !instanceUrl) {
            span.setStatus({ code: SpanStatusCode.ERROR, message: 'Missing token or OAuth request info or instanceUrl' });
