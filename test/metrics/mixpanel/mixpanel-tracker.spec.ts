...
describe("MixpanelTracker", () => {
	...
	describe("constructor", () => {
		it("should create MixpanelTracker instance", () => {
			...
			expect(MixpanelClient).toHaveBeenCalledWith(
				mockSessionInfo.mixpanelToken,
			);
		});
		...
		it("should call identify with userGUID", () => {
			...
			expect(mockMixpanelClient.identify).toHaveBeenCalledWith(
				mockSessionInfo.userGUID,
			);
		});
	...
	describe("track", () => {
		...
		it("should handle multiple consecutive track calls", async () => {
			...
			expect(mockMixpanelClient.track).toHaveBeenNthCalledWith(1, "event1", {
				action: "click",
			});
			expect(mockMixpanelClient.track).toHaveBeenNthCalledWith(2, "event2", {
				action: "submit",
			});
			expect(mockMixpanelClient.track).toHaveBeenNthCalledWith(3, "event3", {
				action: "scroll",
			});
		});
	...
	describe("error scenarios", () => {
		...
		it("should handle MixpanelClient constructor errors", () => {
			...
			expect(() => new MixpanelTracker(mockSessionInfo, mockClient)).toThrow(
