...
import { McpServerError, type Props, getFromKV, putInKV } from "../src/utils";
...
describe("utils", () => {
	...
	describe("putInKV", () => {
		it("should store value in KV when OAUTH_KV is available", async () => {
			...
			expect(mockPut).toHaveBeenCalledWith(testKey, JSON.stringify(testValue), {
		...
		it("should not throw when OAUTH_KV is not available", async () => {
			...
			await expect(
				putInKV("test-key", "test-value", mockEnv),
			).resolves.toBeUndefined();
		});

		it("should not throw when env is undefined", async () => {
			await expect(
				putInKV("test-key", "test-value", undefined as any),
			).resolves.toBeUndefined();
		});
	});
