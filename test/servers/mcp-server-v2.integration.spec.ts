 ...
 *     backed by a real ConversationStorageServerSQLite.  Only the ThoughtSpot API client
 ...
 *     into a real ConversationStorageServerSQLite via a real StorageServiceClient.
...
/**
 * Creates a real ConversationStorageServerSQLite backed by in-memory state, then wraps
...
// ---------------------------------------------------------------------------
// Group 1 — StorageServiceClient ↔ ConversationStorageServerSQLite
// ---------------------------------------------------------------------------

describe("V2 Storage Layer Integration (StorageServiceClient ↔ ConversationStorageServerSQLite)", () => {
...
describe("V2 MCPServer + Real Storage Integration", () => {
	...
	beforeEach(async () => {
		...
		testServer = new MCPServer({ props: mockProps } as any);
			...
			sendAgentConversationMessageStreaming: vi
				...
				.mockImplementation(
					async (
						...
						// so the second send_session_message call below hits the rejection path.
